 ...
 */
void start_thread(unsigned int slave_id)
{
	...
	config->dst_queue = 0;
	rte_eal_remote_launch(run_thread, config, slave_id);
}
...
int main(int argc, char *argv[])
{
	...
	printf("Forwarding between interface %i to interface %i\n", src_interface, dst_interface);

	start_thread(1);
	if (src_interface != dst_interface)
	{
		src_interface ^= dst_interface;
		dst_interface ^= src_interface;
		src_interface ^= dst_interface;
		start_thread(2);
	}
